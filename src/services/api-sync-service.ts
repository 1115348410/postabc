...
export class ApiSyncService {
  ...
  private async syncEnvVariablesToRemote(): Promise<void> {
    try {
      ...
      for (const localEnv of localEnvs) {
        ...
        // 检查远程是否存在相同名称的环境变量
        const existingVar = remoteVariables.find(
          (v) => v.envKey === localEnv.name,
        );
   ...
   */
  private async syncCollectionFromRemote(
    parentUuid: string,
    parentId?: string,
  ): Promise<void> {
    // 获取远程目录列表
    const remoteItems = await this.client.listDirectory(
      parentUuid === "root" ? undefined : parentUuid,
    );
   ...
   * 同步单个文件夹从远程
   */
  private async syncFolderFromRemote(
    folder: ApiInfoDTO,
    parentId?: string,
  ): Promise<void> {
    // 创建本地文件夹
    const localCollection: RequestCollection = {
      ...
      children: [],
      requests: [],
      createdAt: folder.createTime
        ? new Date(folder.createTime).getTime()
        : Date.now(),
      updatedAt: folder.createTime
        ? new Date(folder.createTime).getTime()
        : Date.now(),
    };
   ...
   */
  private async syncApiFromRemote(
    apiInfo: ApiInfoDTO,
    parentId?: string,
  ): Promise<void> {
    try {
      ...
      const localRequest: RequestItem = {
        ...
        createdAt: apiInfo.createTime
          ? new Date(apiInfo.createTime).getTime()
          : Date.now(),
        updatedAt: Date.now(),
   ...
   * 递归同步单个集合到远程 (v2)
   */
  private async syncCollectionToRemote(
    ...
    const existingFolder = remoteItems.find(
      (item) => this.client.isFolder(item) && item.uuid === collection.uuid,
    );
   ...
   */
  private async syncRequestToRemote(
    request: RequestItem,
    parentUuid: string,
  ): Promise<void> {
    if (!request.request) {
      console.warn(
        `[ApiSync] Skipping sync for request ${request.uuid}: request.request is undefined`,
      );
      return;
    }
    ...
    try {
      ...
      const existingApi = remoteItems.find(
        (item) => this.client.isApi(item) && item.uuid === request.uuid,
      );

      // 构建JSON格式字段
      const params =
        typeof request.request.params === "string"
          ? request.request.params
          : JSON.stringify(request.request.params || {});

      const headers =
        typeof request.request.headers === "string"
          ? request.request.headers
          : JSON.stringify(request.request.headers || {});

      const bodyContent =
        typeof request.request.body?.content === "string"
