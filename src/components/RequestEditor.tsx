...
import type { HttpMethod, RequestHeader, QueryParam, BodyType, FormDataField, RequestConfig, ResponseData, StreamConfig } from '../types';
...
type EditorTab = 'params' | 'headers' | 'body' | 'stream' | 'pre-request' | 'test';
...
export default function RequestEditor({ 
  ...
  const [headers, setHeaders] = useState<RequestHeader[]>(initialRequest?.headers || getDefaultHeaders());
  const [queryParams, setQueryParams] = useState<QueryParam[]>(initialRequest?.queryParams || [{ key: '', value: '', enabled: true }]);
  ...
  const [formDataFields, setFormDataFields] = useState<FormDataField[]>(initialRequest?.body?.form || [{ key: '', value: '', type: 'text', enabled: true }]);
  const [urlencodedFields, setUrlencodedFields] = useState<QueryParam[]>(initialRequest?.body?.urlencoded || [{ key: '', value: '', enabled: true }]);
  const [preRequestScript, setPreRequestScript] = useState(initialRequest?.preRequestScript || '');
  const [testScript, setTestScript] = useState(initialRequest?.testScript || '');
  const [streamConfig, setStreamConfig] = useState<StreamConfig>(initialRequest?.streamConfig || DEFAULT_STREAM_CONFIG);
  ...
  const getCurrentRequest = (): RequestConfig => {
    ...
    return {
      ...
      body: bodyType !== 'none' ? {
        ...
        form: bodyType === 'form-data' ? formDataFields.filter((f) => f.enabled && f.key) : undefined,
        urlencoded: bodyType === 'urlencoded' ? urlencodedFields.filter((f) => f.enabled && f.key) : undefined,
      } : undefined,
      preRequestScript,
      testScript,
      timeout: 30000,
  ...
  }, [method, url, headers, queryParams, bodyType, jsonBody, rawBody, formDataFields, urlencodedFields, preRequestScript, testScript, streamConfig]);
  ...
  const handleSendRequest = useCallback(async () => {
    ...
    if (streamConfig.enabled && streamConfig.extractionRules.length > 0) {
      streamExtractorRef.current = new StreamingFieldExtractor(streamConfig.extractionRules);
    } else {
    ...
    try {
          ...
          if (data.type === 'sse-event') {
            ...
            if (streamExtractorRef.current) {
              ...
              if (jsonData) {
                ...
                extractedFields = streamExtractorRef.current.getAccumulatedFields();
              }
            }
      ...
      if (result.success && result.data) {
        ...
        if (result.data.body?.type === 'sse') {
          const finalExtractedFields = streamExtractorRef.current?.getAccumulatedFields();
          flushSync(() => {
            const sseResponse = {
              ...result.data,
              body: {
                ...result.data.body,
                extractedFields: finalExtractedFields || result.data.body?.extractedFields,
              },
            };
  ...
  }, [method, url, environment, headers, queryParams, bodyType, jsonBody, rawBody, formDataFields, urlencodedFields, streamConfig]);
  ...
  const handleUpdateRequest = useCallback(async () => {
    ...
    try {
      ...
      if (bodyType === 'json') {
        ...
        bodyContent = JSON.stringify(formDataFields.filter(f => f.enabled && f.key));
      } else if (bodyType === 'urlencoded') {
        bodyContent = JSON.stringify(urlencodedFields.filter(f => f.enabled && f.key));
      }
  ...
  }, [apiUuid, url, method, headers, queryParams, bodyType, jsonBody, rawBody, formDataFields, urlencodedFields, preRequestScript, testScript, streamConfig]);
  ...
  const loadRequest = (request: RequestConfig) => {
    ...
    setQueryParams(request.queryParams?.length ? request.queryParams : [{ key: '', value: '', enabled: true }]);
  ...
  const tabs: { id: EditorTab; label: string; count?: number }[] = [
    ...
    { id: 'body', label: 'Body' },
    { id: 'stream', label: 'Stream', count: streamConfig.enabled && streamConfig.extractionRules.length > 0 ? streamConfig.extractionRules.length : undefined },
    { id: 'pre-request', label: '前置脚本', count: preRequestScript ? 1 : undefined },
