...
export default function Sidebar({
  ...
  const [parentFolderUuid, setParentFolderUuid] = useState<string | undefined>(
    undefined,
  );
  const [editingFolderUuid, setEditingFolderUuid] = useState<string | null>(
    null,
  );
          ...
          switch (detail.bodyType) {
            ...
            case "urlencoded":
              try {
                ...
                body = {
                  urlencoded: Array.isArray(urlencodedData)
                    ? urlencodedData
                    : [],
                };
              } catch (e) {
  ...
  const rootItems = items.filter(
    (item) => item.parentUuid === "root" || !item.parentUuid,
  );
  ...
  const renderApi = (api: ApiInfoDTO) => {
    ...
    return (
      ...
      >
        <span
          className={`text-xs font-mono font-bold min-w-[50px] text-center ${methodColorClass}`}
        >
          {method.toUpperCase()}
        </span>
