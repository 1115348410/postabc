...
import type { HttpMethod, RequestHeader, QueryParam, BodyType, FormDataField, RequestConfig, StreamConfig } from '../types';
import StreamConfigEditor from './request-builder/StreamConfigEditor';

type RequestTab = 'params' | 'headers' | 'body' | 'stream' | 'pre-request' | 'test';
...
export default function DevToolsPanel() {
  ...
  const loadEnvironmentVariables = async () => {
    ...
    if (Object.keys(cachedVars).length > 0) {
      ...
      const variables = await storageAPI.getActiveEnvironmentVariables('global');
  ...
  const handleSendRequest = useCallback(async () => {
    ...
    const request: RequestConfig = {
      ...
      body: bodyType !== 'none' ? {
        ...
        form: bodyType === 'form-data' ? formDataFields.filter((f) => f.enabled) : undefined,
        urlencoded: bodyType === 'urlencoded' ? urlencodedFields.filter((f) => f.enabled) : undefined,
      } : undefined,
      preRequestScript,
      testScript,
      timeout: 30000,
    ...
    if (!validation.isValid) {
      setResponse({
        ...
        request: {
          ...
          url: request.url,
          headers: (request.headers as Array<{key: string; value: string}>).reduce((acc, h) => ({ ...acc, [h.key]: h.value }), {} as Record<string, string>),
    ...
    console.log('[DevToolsPanel] 环境变量 keys:', Object.keys(environment || {}));
  ...
  const tabs: { id: RequestTab; label: string; count?: number }[] = [
    { id: 'params', label: 'Params', count: queryParams.filter((p) => p.enabled).length },
    ...
    { id: 'pre-request', label: 'Pre-request', count: preRequestScript ? 1 : undefined },
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">API数据同步</h2>
              <button
